/* Shared client-side validation helpers for PAMS (restored)
   This file validates forms: registerForm, loginForm, profileForm
*/
(function(){
    const patterns = {
        email: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
    ...
    function setError(el, message){
        ...
        if(message){ el.classList.add('input-invalid'); if(err) err.textContent = message; }
        else { el.classList.remove('input-invalid'); if(err) err.textContent = ''; }
    }
